...
public class CountLineWordsDuplicateWords {
	...
	public CountLineWordsDuplicateWords() {
		...
		try {
			...
			// System.out.println("The total number of words in the text file is
			// "+arrayLength);
				...
				for (int j = i + 1; j < arrayLength; j++) {
					if (stringArray[i].equalsIgnoreCase(stringArray[j])) {
						...
						if (stringArray[i].equalsIgnoreCase(wordname)) {
							System.out.println(
									"The word " + wordname + " is present " + c + " times in the specified file.");
						}

					} // End of Inner for block
					// System.out.println("The word "+stringArray[i]+" is
					// present "+c+" times in the specified file.");
